...
(() => {
  ...
  // src/gemini_model.ts
  var import_custom_node_types = __require("@visualblocks/custom-node-types");
  var import_node_utils = __require("@visualblocks/node-utils");
  ...
  var NODE_SPEC = {
    id: "gemini-model-2.5",
    name: "Gemini Language Model 2.5",
    description: "Send queries and fetch responses from Google's Gemini models.",
    category: import_custom_node_types.Category.MODEL,
    inputSpecs: [
      {
        name: "prompt",
        displayLabel: "Prompt",
        type: import_custom_node_types.DataType.ANY,
        // TODO: Better types. Union(DataType.STRING, DataType.IMAGE).
        multiple: true,
        editorSpec: {
          type: import_custom_node_types.EditorType.TEXT_AREA,
          autoResize: true
        }
      },
      {
        ...
        type: import_custom_node_types.DataType.STRING,
        editorSpec: {
          type: import_custom_node_types.EditorType.DROPDOWN,
          options: ["gemini-2.5-flash", "gemini-2.5-pro"].map(
            (l) => ({ label: l, value: l })
          )
        }
      },
      {
        ...
        noSave: true,
        editorSpec: {
          type: import_custom_node_types.EditorType.TEXT_INPUT,
          password: true
        }
      }
    ],
  ...
  var GeminiModel = class _GeminiModel extends import_node_utils.PureFunctionNode {
    genAI;
    model;
    constructor() {
      super();
    ...
    async run(inputs, services) {
      ...
      const parts = [...flattened].map((prompt, i) => {
  ...
  // src/entry_gemini_only.ts
  visualblocks.registerCustomNode({
    nodeSpec: GeminiModelInfo.nodeSpec,
    nodeImpl: GeminiModelInfo.nodeImpl
  });
})();
/**
 * @license
